...
Arduino Pro mini sketch for controlling the lights in project 'Transformatorstation (Kibri 9840)'

Uses three touch sensors connected to the Arduino to control three LEDs that simulate the arc flash in the transformer station.

When switched on, the arc simulation is active on all three LEDs.
After switching on, the arc simulation is switched on at random times.
The arc flash simulation can also be activated by putting your finger close to a strategically placed touch sensor. 
